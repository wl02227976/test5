...
class TD3(object):
	...
	def step(self, state, action, reward, next_state, done):
		...
		##print('test')
		self.replay_buffer.add((state, action, reward, next_state, done), reward)
	...
	def train(self, replay_buffer, batch_size=1024):
		...
		nnncurrent_Q1, current_Q2 = self.critic(state, action)

		# Compute critic loss
		##critic_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(current_Q2, target_Q)
		critic_loss = self.mse(current_Q1, target_Q, is_weights) + self.mse(current_Q2, target_Q, is_weights)
